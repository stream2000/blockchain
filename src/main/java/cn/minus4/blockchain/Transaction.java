...
public class Transaction {
    ...
    public static boolean verifySignature(Transaction t, byte[] signature) {
        String data = StringUtil.getStringFromKey(t.getSender()) + StringUtil.getStringFromKey(t.getRecipient()) + 0;
    ...
    private void generateSignature(PrivateKey privateKey) {
        String data = StringUtil.getStringFromKey(sender) + StringUtil.getStringFromKey(recipient) + 0;
        signature = StringUtil.applyECDSASig(privateKey, data);
    }

    public String calculateHash() {
        return StringUtil
            .applySha256(StringUtil.getStringFromKey(sender) + StringUtil.getStringFromKey(recipient) + new Date().getTime());
    }
